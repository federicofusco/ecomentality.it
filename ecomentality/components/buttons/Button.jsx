...
const Button = ({ onClick, children, rounded = true, disabled = false }) => {
	...
	return <button onClick={ click } className={`${ disabled ? "text-gray-200 bg-green-800 cursor-not-allowed" : "" } text-white font-medium bg-green-600 ${ rounded ? "rounded-full" : "rounded-md" } py-1 px-3 text-sm ml-auto`}>
		{ children }
	</button>
}
